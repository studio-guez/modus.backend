    ...
    switch ($itemType) {
        case 'page':
            ...
            if ($linkedPage) {
                ...
                // Build query params based on linked page's template
                $queryParams = [];
                $pageTemplate = $linkedPage->intendedTemplate()->name();
                
                // Get filter values
                $projectTypeFilter = $item->projectTypeFilter()->value();
                $statusFilter = $item->statusFilter()->value();
                $mediaTypeFilter = $item->mediaTypeFilter()->value();
                
                // Apply filters based on page template
                switch ($pageTemplate) {
                    case 'projects':
                        if (!empty($projectTypeFilter)) {
                            $queryParams['typeFilter'] = $projectTypeFilter;
                        }
                    ...
                    case 'medias':
                        if (!empty($mediaTypeFilter)) {
                            $queryParams['typeFilter'] = $mediaTypeFilter;
                        }
                        break;
                    case 'tags':
                        if (!empty($statusFilter)) {
                ...
                if (!empty($queryParams)) {
                    $result['url'] .= '?' . http_build_query($queryParams);
                }
            }
            break;
